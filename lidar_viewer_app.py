...
class SerialReaderThread(QThread):
    ...
    def __init__(self, port, baudrate=115200, protocol_config=None):
        super().__init__()
        self.port = port
...
class FileReaderThread(QThread):
    ...
    def __init__(self, filepath, speed_mult=1.0, protocol_config=None):
        super().__init__()
...
class LidarViewerWindow(QMainWindow):
    ...
    def _create_display_toolbar(self):
        ...
        self._range_spin.setRange(1, 50)
    ...
    def _connect_serial(self):
        ...
        self._reader_thread = SerialReaderThread(port, baud, self._protocol_config)
    ...
    def _start_replay(self, path):
        ...
        self._file_thread = FileReaderThread(path, speed, self._protocol_config)
    ...
    def _rebuild_scan_from_loaded(self):
        ...
        与正常播放逻辑一致：取最近 frames_per_scan 帧的 valid_points。
        """
        ...
        # 取最近 fps 帧的 valid_points（与播放逻辑一致）
        start = max(0, idx - fps)
        all_points = []
    ...
    def _on_unit_changed(self, unit):
        ...
        ranges = {"mm": (1, 5000), "cm": (1, 500), "m": (1, 50)}
        lo, hi = ranges.get(unit, (1, 5000))
